...
// Problem 1
//The event director needs email address of each runner.  Combine all email address into a new array called allemailaddress. 

let allemailaddress = runners.map(runner => {
    return runner.email;
});
console.log(allemailaddress); 

// Problem 2
//The event director needs the company name of each runner.  Combine all company names into a new array called allcompanyname.

let allcompanyname = [];
runners.forEach(runner => {
    allcompanyname.push(runner.company_name);
});
console.log(allcompanyname);


// Problem 3
// The event director needs know of each runner's donation over 100 dollar  It will into a new array called overdonation.

let overdonation = runners.filter(runner => {
    return runner.donation > 100; 
}); 
console.log(overdonation);
